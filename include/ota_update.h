...
// based on https://github.com/espressif/arduino-esp32/blob/master/libraries/Update/examples/AWS_S3_OTA_Update/AWS_S3_OTA_Update.ino
...
void do_ota_upgrade(String host, int port, String bin, int command = U_FLASH) {
    ...
    */
    while (client.available()) {
      ...
      if (line.startsWith("Content-Type: ")) {
        String contentType = get_header_value(line, "Content-Type: ");
  ...
  if (contentLength && isValidContentType) {
    ...
    if (canBegin) {
      ...
      Serial.println("Begin writing stream to updater");
      size_t written = Update.writeStream(client);
      Serial.println("Done writing stream to updater");
